...
public class BarChartEntryData: ObservableObject {
    /// タイトル
    @Published var legend: LocalizedType = .CoopHistory_Title
    /// エントリー
    @Published var entries: [BarChartEntry] = []

    init() {}

    init(legend: LocalizedType, entries: [BarChartEntry]) {
        self.legend = legend
        self.entries = entries
    }

    convenience init(legend: LocalizedType, entries: BarChartEntry) {
        self.init(legend: legend, entries: [entries])
    }
}

/// チャートのエントリセット
public struct BarChartEntrySet {
    public typealias ChartEntryType = BarChartEntry

    /// ラベル
    public var label: String = ""
    /// データセット
    public var data: [BarChartEntry]

    public init(label: String, data: [BarChartEntry]) {
        self.label = label
        self.data = data
    }

    public init(label: LocalizedType, data: [BarChartEntry]) {
...
//
//public extension BarChartEntryData {
//    /// xの最大値
//    func xMax() -> Double {
